...
g=9.81
qAço=roAço*A*g
qAlumínio=roAlumínio*A*g
F=320*g
n=[4,8,16,32]
enorm = []
for n_i in n:
    print(f"\n{'='*50}")
    print(f"Processando: n = {n_i} elementos")
    print(f"{'='*50}")
    h = L / n_i
    aux = 0
    mesh=Mesh(dim=1)
    ...
    for i in range(n_i):
        ...
        node1_coord=node1.coord[0]
        node2_coord=node2.coord[0]
        nodevec=[node1,node2]
        if node2_coord<=0.25:
            element=Bar2Node(A=A,E=Eaço,nodevec=nodevec,b=qAço)
            mesh.addEl(element)
        elif node1_coord>=0.25 and node2_coord<=0.5:
            element=Bar2Node(A=A,E=Ealumínio,nodevec=nodevec,b=qAlumínio)
            mesh.addEl(element)
        elif node1_coord>=0.5 and node2_coord<=0.75:
            element=Bar2Node(A=A,E=Eaço,nodevec=nodevec,b=qAço)
            mesh.addEl(element)
        elif node1_coord>=0.75:
            element=Bar2Node(A=A,E=Ealumínio,nodevec=nodevec,b=qAlumínio)
            mesh.addEl(element)
    bc1=BarBC(nodevec=[nodes_list[0]],type="Displacement",val=[0.0],E=Eaço,A=A)
    bc2=BarBC(nodevec=[nodes_list[-1]],type="Load",val=[F],E=Ealumínio,A=A)
    mesh.addEl(bc1)
    mesh.addEl(bc2)
