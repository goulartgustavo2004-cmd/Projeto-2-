...
# ------------------------------------------------
  ...
  def calcstiff(self)->tuple[np.ndarray,np.ndarray]:
    ...
    intrule = self.intrule()
    # Initialize kel and fel with zeros and the correct dimensions
    kel = np.zeros((self.ndofs(), self.ndofs()))
    fel = np.zeros(self.ndofs())
    # Loop over all integration points
    for qsi, weight in zip(intrule.x, intrule.w):
      # Compute the shape functions and their derivatives
      N, dNdqsi = self.shape(qsi)
      jac, invjac, dNdx = self.jacobian(dNdqsi)
      # Compute the B matrix
      B = self.createB(dNdx)
      # Add contribution of the integration point to the element stiffness matrix and load vector
      kel += B.T @ B * (self.E * jac * weight*self.A)
    # stiffness matrix and load vector
      fel += np.array(N) * self.b * jac * weight
    return kel, fel       
