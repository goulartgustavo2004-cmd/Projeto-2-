  ...
  def assemble(self):
    ...
    for el in self.mesh.elvec:
      ...
      if len(el.nodevec) == 2:
        ...
        print(f"Element {el.index} stiffness matrix:\n{kel}")
        print(f"Element {el.index} load vector:\n{fel}")
...
# ------------------------------------------------
  ...
  def computeEnergyNormError(self, Uex: float)->float: 
    K_glob = np.zeros((self.mesh.nnodes()*self.mesh.dim,self.mesh.nnodes()*self.mesh.dim))
    for el in self.mesh.elvec:
      el.buildEFT()
      if len(el.nodevec) == 2:
        kel, fel = el.calcstiff()
        for i in range(el.ndofs()):
          for j in range(el.ndofs()):
            K_glob[el.eft[i], el.eft[j]] += kel[i, j]
    U_h = 0.5 * self.u.T @ K_glob @ self.u
    if Uex == 0:
      DebugStop("Uex cannot be zero when computing the energy norm error")
    Uex_abs = abs(Uex)
    error = np.sqrt(np.abs((Uex_abs - U_h) / Uex_abs))
