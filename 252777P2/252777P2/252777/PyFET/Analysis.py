...
# Provide a local wrapper for calcstiff to support different element implementations
def calcstiff(el):
  """Wrapper to compute element stiffness and load vector.
  Tries common method names on element instances.
  """
  if hasattr(el, 'calcstiff') and callable(getattr(el, 'calcstiff')):
    return el.calcstiff()
  if hasattr(el, 'calcStiff') and callable(getattr(el, 'calcStiff')):
    return el.calcStiff()
  # Fallback: if element module provided a free function, try to use it
  try:
    from .Element import calcstiff as _calc
    return _calc(el)
  except Exception:
    raise NameError(f"calcstiff not found for element of type {type(el)}")
