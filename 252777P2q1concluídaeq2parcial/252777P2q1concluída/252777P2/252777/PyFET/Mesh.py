...
# ------------------------------------------------
  ...
  def generateGeometryVTK(self, filename: str):
    ...
    # 2. Print nodes
    f.write(f"Points {self.nnodes()} float\n")
    for node in self.nodevec:
      if len(node.coord) == 2:
        f.write(" ".join(np.append(node.coord,0.).astype(np.str_)) + "\n")
      else:
        f.write(" ".join(node.coord.astype(np.str_)) + "\n")
