...
# ------------------------------------------------
  ...
  def buildEFT(self):
    ...
    self.eft = np.zeros(ndofs, dtype=int)
    ...
    for i in range(self.nnodes()):
      for j in range(self.nstatevar()):
        self.eft[i*self.nstatevar()+j] = self.nodevec[i].index*self.nstatevar() + j
# ------------------------------------------------
# ------------------------------------------------
  ...
  def calcstiff(self)->tuple[np.ndarray,np.ndarray]:
    ...
    x, w = self.intrule().rule
    # Initialize kel and fel with zeros and the correct dimensions
    ndofs=self.ndofs()
    kel = np.zeros((ndofs, ndofs))
    fel = np.zeros((ndofs, 1))
    # Loop over all integration points
    for i in range(self.intrule().numPoints()):
      xi = xivec[i]
      wi = wvec[i]
      N,dNdxi=self.shape(xi)
      jac, invjac, detjac = self.jacobian(dNdxi)
      dNdx=self.physicalderivatives(dNdxi, invjac)
      B=self.createB(dNdx)
      kel+=self.Dmat*B.T@B*detjac*wi
      fel+=self.b*N.T*detjac*wi
